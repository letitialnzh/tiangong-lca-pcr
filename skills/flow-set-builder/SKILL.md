...
Use the binding policy declared by the current draft Flow Set. A policy may allow
set-level binding, group-level binding, or both, and declares its default.
Group-level bindings use one narrowest applicable group. Set-level bindings
cite the registry without a group and may define a consolidation or expansion
rule for PCR cards. `energy-supply` defaults to set-level when foreground
records determine the actual carrier and permits one exact group when the PCR
method requires a specific energy function. Only when the registry does not
cover the exchange may the PCR resolve an exact verified platform UUID
(`fixed`). Candidate UUIDs support review but never determine the PCR category.
Final process data still resolves every parameterized record to one concrete
UUID.
...
2. Apply the current draft V2 Flow Set's declared binding policy and default level.
   Select one narrowest applicable group when group-level binding is selected.
   Cite only the set id and version when set-level binding is selected, and
   follow any PCR-card consolidation or expansion rule. The PCR does not
   resolve a UUID at authoring time.
...
6. If the use is ambiguous, return `needs_review`; do not guess from the
   ...
   must retain the Flow Set version and the downstream selection evidence.
...
selection_policy:
  ...
  pcr_cardinality: multiple_per_process
