...
const coordinateFor = (set) => set.coordinate ?? defaultCoordinate;
const requiredCoordinates = new Set((taxonomy.sets ?? []).map(coordinateFor).filter(Boolean));
const needsProduct = [...requiredCoordinates].some((coordinate) => coordinate.startsWith("product-"));
const needsElementary = [...requiredCoordinates].some((coordinate) => coordinate.startsWith("elementary-"));
...
const missingInputs = [
  ...(needsProduct && !productPath ? ["--product=<product-snapshot.json>"] : []),
  ...(needsElementary && !elementaryPath ? ["--elementary=<elementary-snapshot.json>"] : []),
];
if (missingInputs.length) {
  console.error(`Missing snapshot input(s) required by taxonomy scope: ${missingInputs.join(", ")}`);
  process.exit(2);
}
...
const products = needsProduct ? readRows(productPath).map(toCandidate) : [];
const elementary = needsElementary ? readRows(elementaryPath).map(toCandidate) : [];
...
const sourcePaths = [needsProduct ? productPath : null, needsElementary ? elementaryPath : null].filter(Boolean).sort();
for (const filePath of sourcePaths) sourceHash.update(readFileSync(filePath));
...
const allGroups = taxonomy.sets.flatMap((set) => set.groups.map((group) => ({ ...group, setId: set.id, coordinate: coordinateFor(set) })));
...
for (const set of taxonomy.sets) {
  const coordinate = coordinateFor(set);
  const pool = coordinate.startsWith("product-") ? productPool : elementaryPool;
  const bindingLevel = set.binding_level ?? "group";
  const allowedBindingLevels = set.allowed_binding_levels ?? [bindingLevel];
  const groupRequired = set.group_required ?? !allowedBindingLevels.includes("set");
  const groups = set.groups.map((group) => {
    const rows = uniqueByIdentity(pool.filter((row) => matchGroup({ ...group, coordinate }, row)));
    const candidates = rows.map((row) => candidateFor(row, { ...group, coordinate }));
    ...
    return {
      ...
      include_when: [`The exchange has the ${group.id} function.`, `The flow has coordinate ${coordinate}.`],
      ...
      required_project_data: ["quantity", "property", "unit", ...(coordinate === "elementary-output" ? ["receiving_medium", "reported_substance"] : [])],
      selection_constraints: {
        properties: coordinate === "elementary-output" ? ["mass"] : ["mass", "amount", "energy", "area", "mass*distance"],
        units: ["kg", "g", "MJ", "kWh", "kg*km", "t*km"],
        geography_policy: coordinate === "elementary-output" ? "not_applicable_for_elementary_flow" : "prefer_process_location",
      },
  ...
  const output = {
    schema_version: 1,
    identity: { id: `flow-set.${set.id}`, version: String(set.version ?? "0.2.0"), status: "draft", title: `${set.id} functions (V2)` },
    scope: { flow_type: coordinate.startsWith("elementary") ? "elementary" : "product", direction: coordinate.endsWith("output") ? "output" : "input", coordinate, function: genericDescriptions[set.id] },
    selection_policy: {
      bind_at: "foreground_data_generation",
      binding_level: bindingLevel,
      allowed_binding_levels: allowedBindingLevels,
      ...(set.pcr_cardinality ? { pcr_cardinality: set.pcr_cardinality } : {}),
      final_exchange_requires_verified_uuid: true,
      automatic_selection_allowed: false,
      priority: [allowedBindingLevels.length > 1 ? "applicable_flow_set_binding" : bindingLevel === "set" ? `${set.id.replaceAll("-", "_")}_set` : "applicable_flow_set_group", "exact_verified_uuid_for_uncovered_flow", "unmapped_coverage"],
  ...
  const populationLines = coordinate.startsWith("product-")
...
console.log(JSON.stringify({
  ...
  ...(needsProduct ? { product_rows: productPool.length } : {}),
  ...(needsElementary ? { elementary_rows: elementaryPool.length } : {}),
  results,
}, null, 2));
