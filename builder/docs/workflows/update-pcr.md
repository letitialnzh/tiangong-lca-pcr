...
5a. When the process map or inventory changes, derive and complete all Process Map flow cards before binding identity. Do not create or split cards by enumerating Flow Set groups; split only for independently required inventory, measurement, calculation, boundary, condition, or validation behavior. Match a semantically concrete card to the narrowest applicable reviewed group. A conditional umbrella card may cite only the set id/version and defer case-specific group selection. Only a flow not covered by an applicable set is refined to the lowest evidence-supported level using material or substance identity, flow type, direction, process state or gate, intended use, and destination, then sent to platform lookup.
5b. Use `parameterized` for a Flow Set reference, with one exact group for a concrete card and no group for a deferred umbrella card. Never encode alternative ids in `group`. Use `fixed` only for an exact verified UUID of an uncovered flow; otherwise retain unmapped coverage. Foreground data generation must expand deferred scope from actual records and resolve each emitted exchange to one group and UUID. Do not change the real boundary, flow type, or direction to force a match, use a product-output Flow Set fallback, or create a new Flow Set/module during this pass. Record any reusable-pattern module gap for separate forward module design.
