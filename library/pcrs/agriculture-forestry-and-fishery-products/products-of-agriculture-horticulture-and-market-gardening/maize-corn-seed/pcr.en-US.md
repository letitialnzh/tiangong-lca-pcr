...
This PCR guides a foreground data package for maize seed intended for sowing under CPC 3.0 `01121`. It covers seed multiplication or controlled seed production, harvest, seed conditioning, grading, quality testing, optional treatment, packaging, and optional storage to a declared seed gate. The default gate is the seed plant gate; a delivery gate is allowed only when storage and delivery records are included.
It applies to open-pollinated, hybrid, foundation, registered, certified, or comparable maize sowing-seed routes when seed class, production route, quality status, geography, and gate are declared. Parental-line isolation, detasseling, controlled pollination, and certification activities are included when inside the foreground boundary, but this PCR supplies no universal breeding facts, quantities, factors, UUIDs, or certification results.
Commodity maize grain, forage or silage maize, sweet or baby corn sold as a vegetable, food or feed processing, starch or ethanol conversion, downstream cultivation, retail distribution, and consumer use are excluded. Breeding research and capital goods are excluded unless the data package explicitly includes them.
...
| classification_refs | CPC 3.0 `01121`, `Maize (corn), seed` |
| covered_products | Cleaned, graded, quality-controlled maize seed intended for sowing, including declared hybrid and non-hybrid seed classes |
| excluded_products | Commodity maize grain; forage or silage maize; sweet or baby corn; processed maize products; downstream crop production; unqualified breeding research |
| representative_product | Accepted maize sowing seed at declared moisture basis, quality class, treatment status, packaging state, and gate |
| production_route | Managed maize seed multiplication or controlled seed production followed by harvest, conditioning, quality testing, optional treatment, and packaging |
| market_state | Seed lot accepted for sowing and transferred at the declared seed plant or delivery gate |
## 3. Reference Flow
| Field | Value |
| --- | --- |
| What | Accepted maize seed for sowing at the declared seed gate |
| How much | 1 kg net accepted seed |
| How well | Seed class or certification class, hybrid or open-pollinated status, variety or parental-line identity where applicable, moisture basis, physical purity, germination or viability, treatment status, packaging state, geography, and gate |
| How long or cycle | One declared seed production crop cycle; storage duration is declared when storage is included |
...
| Reference unit | kg |
| Required qualifiers | seed class or certification class; hybrid or open-pollinated status; variety or parental-line identity where applicable; treatment status; moisture basis; physical purity; germination or viability; geography; crop cycle; declared gate; packaging state |
| Binding | `fixed` |
...
| --- | --- | --- | --- | --- |
| `reference_mass` | reference product | Mass `93a60a56-a3c8-11da-a746-0800200c9a66` | kg | Express the reference as net accepted maize sowing seed at the declared gate and exclude container tare. |
...
| Field | Value |
| --- | --- |
| declared_starting_condition | source seed lot, parental-line lot, or declared seed-production starting material |
| starting_condition_role | reproduction identity and crop-production starting condition |
| product_classification_scope | CPC 3.0 `01121`, maize seed for sowing; commodity grain, forage or silage maize, and downstream maize products are outside this PCR |
...
| disclosure | seed class; hybrid or open-pollinated route; variety or parental-line identity; field and geography; crop cycle; irrigation; nutrient and crop-protection records; harvest mass and moisture; conditioning, testing, treatment, packaging, storage, rejects, and gate |

| rule_id | Applies to | Rule | source_ids |
| --- | --- | --- | --- |
| `boundary_field_to_seed_gate` | foreground boundary | Include seed production, harvest, conditioning, quality testing, optional treatment, packaging, and only storage or delivery needed to reach the declared seed gate. | `fao-maize-seed-production`; `fao-maize-drying-handling` |
| `boundary_gate_definition` | reference product | Use the seed plant gate by default. A delivered-seed gate requires storage or delivery records, losses, and gate hand-off. | `fao-maize-seed-production`; `mass-balance-identity` |
| `boundary_seed_quality` | accepted seed | Include grading, quality testing, treatment status, and lot acceptance when they determine whether the output is accepted sowing seed. | `fao-maize-seed-production`; `usda-seed-cleaning-handling` |
...
| `boundary_output_separation` | all outputs | Separate accepted seed, intermediate harvested seed crop, rejects, screenings, dust, residues, packaging waste, and unexplained loss by measured mass and fate. | `mass-balance-identity`; `fao-maize-drying-handling` |
## 6. Process Inventory Structure
### Process Map
| process_id | process_name | inclusion | inclusion_condition | role | quantitative_reference |
| --- | --- | --- | --- | --- | --- |
| `field_maize_seed_multiplication` | Field Maize Seed Multiplication | required |  | foreground | crop-cycle inputs and harvested seed crop before conditioning |
| `seed_conditioning_treatment_and_packaging` | Seed Conditioning, Treatment, and Packaging | required |  | foreground | accepted cleaned and packaged maize sowing seed |
...
### Process: Field Maize Seed Multiplication (`field_maize_seed_multiplication`)
#### Inputs
##### Product flows
###### Source seed or parental-line lot (`source_seed_or_parental_line_lot`)
Record the source lot that establishes the declared maize seed route. Identity, class, treatment, mass, and source field are separate from current-cycle accepted output.
- Selected flow: maize source seed or parental-line lot used for sowing 23f01fdc-33b5-4cc9-bd0f-b0cb39d7daa6
- Flow property / unit: Mass / kg
- Amount rule: measured source-lot mass by field or parental-line unit
...
- Sources: `fao-maize-seed-production`
###### Agricultural nutrient and fertilizer inputs (`field_maize_seed_multiplication_agricultural_nutrient_inputs`)
...
#### Outputs
##### Product flows
###### Harvested maize seed crop for conditioning (`harvested_maize_seed_crop_for_conditioning`)
...
- Selected flow: harvested maize seed crop for conditioning
- Flow property / unit: Mass / kg
...
- Sources: `fao-maize-seed-production`; `mass-balance-identity`
##### Waste flows
...
- Selected flow: harvested maize seed crop received for conditioning 23f01fdc-33b5-4cc9-bd0f-b0cb39d7daa6
- Flow property / unit: Mass / kg
...
- Sources: `mass-balance-identity`; `fao-maize-drying-handling`
...
#### Outputs
##### Product flows
###### Accepted maize sowing seed (`accepted_maize_sowing_seed`)
Accepted cleaned, graded, tested, optionally treated, and packaged seed is the final reference product at the declared seed gate.
...
#### Outputs
##### Product flows
###### Delivered accepted maize seed (`delivered_accepted_maize_seed`)
Use only when the declared gate is after storage. It remains the same reference-product function and must not be counted with seed-plant-gate output.
- Selected flow: Corn Seeds `23f01fdc-33b5-4cc9-bd0f-b0cb39d7daa6`
- Flow property / unit: Mass / kg
- Binding: `fixed`
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `cp_source_seed_lot_records` | `field_maize_seed_multiplication` | source seed or parental-line lot | seed lot record | lot; class; hybrid or parental-line role; treatment; mass; moisture; supplier or source field | lot certificate, receiving record, calibrated scale | kg and declared attributes | per lot | crop cycle | field or production unit | sum by field or parental-line unit and normalize to harvested seed crop | certificate, receiving record, scale |
| `cp_nutrient_input_records` | `field_maize_seed_multiplication` | nutrient and amendment inputs | application record | product; formulation; nutrient content; mass; date; field; method | invoice, label, application log, service record | kg product and kg nutrient | per application | crop cycle | field | sum product and nutrient mass by field | invoice, label, application record |
| `cp_irrigation_records` | `field_maize_seed_multiplication` | irrigation and withdrawal | meter or source record | source; meter; reading; volume; date; field; pumping energy | calibrated meter, pump log, water record | m3, kg, kWh | event or month | crop cycle | field and source | sum by source and distinguish delivered water from withdrawal | calibration, readings, source record |
| `cp_field_energy_records` | `field_maize_seed_multiplication` | field fuel and electricity | fuel or utility record | operation; carrier; quantity; machine or supplier; date; field | fuel log, meter, invoice, contractor record | L, kg, MJ, kWh | operation or billing interval | crop cycle | field or unit | attribute energy without double counting | meter, invoice, log |
| `cp_crop_protection_records` | `field_maize_seed_multiplication` | crop protection | application record | product; active substance; dose; area; date; field; target | label and application log | kg product, kg active substance, ha | per application | crop cycle | field | sum by product and active substance | label, applicator record |
| `cp_land_use_records` | `field_maize_seed_multiplication` | land occupation | field area record | field; planted area; dates; crop duration | field record, GIS, farm register | ha and crop cycle | per cycle | crop cycle | field | calculate area-time and normalize | field map, GIS, farm record |
| `cp_harvested_seed_mass` | `field_maize_seed_multiplication` | harvested seed crop | harvest record | field; date; mass; moisture; seed route; rejects | calibrated scale and harvest record | kg and moisture fraction | per harvest | crop cycle | field and receiving point | reconcile before conditioning | scale, harvest record |
| `cp_field_residue_records` | `field_maize_seed_multiplication` | field residue | residue record | stream; mass; fate; date; field | scale, baling ticket, field log | kg | per event | crop cycle | field | sum by fate and boundary crossing | scale, ticket, field log |
...
| `convert_seed_moisture_basis` | field and conditioning mass | convert only from recorded mass and moisture using an explicit water-and-dry-matter equation | harvest; receiving; quality records | declared-basis mass | `mass-balance-identity`; `fao-maize-drying-handling` |
...
| `calculate_conditioning_energy_emissions` | conditioning energy | multiply recorded conditioning fuel or energy by the declared factor and normalize | conditioning energy; factor | conditioning emissions | `fao-maize-drying-handling` |
...
| `reconcile_conditioning_outputs` | conditioning | received mass and moisture reconcile to accepted seed, rejects, packaging waste, emissions, and documented loss | conditioning input; accepted seed; rejects | conditioning balance | `mass-balance-identity`; `fao-maize-drying-handling` |
...
| --- | --- | --- | --- |
| `dq_seed_identity` | reference product and source lots | Declare seed class, hybrid or open-pollinated status, variety or parental-line identity where applicable, treatment status, moisture basis, physical purity, and germination or viability. | certificate, lot record, quality test |
...
| rule_id | Applies to | Rule | source_ids |
| --- | --- | --- | --- |
| `validation_reference_flow` | reference product | Confirm the verified maize sowing-seed UUID, all qualifiers, and the declared gate; the final exchange must retain that UUID. | `mass-balance-identity` |
| `validation_scope` | boundary | Confirm that the dataset is maize seed for sowing and does not silently include commodity grain, forage or silage maize, downstream cultivation, or downstream processing. | `fao-maize-seed-production` |
| `validation_process_map` | process inventory | Confirm field seed production, conditioning, quality release, and included storage are separate, with explicit hand-offs. | `fao-maize-seed-production`; `usda-seed-cleaning-handling` |
...
| `validation_quality_release` | accepted seed | Confirm seed class, quality attributes, treatment status, lot release, packaging state, and gate for the reference output. | `fao-maize-seed-production` |
...
| excluded_use | commodity grain; forage or silage maize; downstream cultivation; food, feed, starch, or ethanol processing; datasets omitting seed-quality or gate qualifiers |
| required_metadata | reference flow; CPC scope; seed class; hybrid or open-pollinated status; variety or parental-line identity where applicable; geography; crop cycle; moisture; purity; germination or viability; treatment; packaging; gate; Flow Set selections; collection coverage; DQR |
...
| update_trigger | material change in route, hybrid or parental-line practice, conditioning technology, treatment, packaging, storage, reference qualifiers, Flow Set contract, collection coverage, or data quality |
## 11. Data Sources
| Source id | Type | Reference | Used for |
| --- | --- | --- | --- |
| `fao-maize-seed-production` | official_guidance | FAO seed production and quality guidance, <https://www.fao.org/4/y4011e/y4011e0v.htm> | seed classes, quality attributes, lot acceptance, and seed-production boundary |
| `fao-maize-harvest-postharvest-2023` | official_guidance | FAO maize harvesting and post-harvesting handling, <https://www.fao.org/family-farming/detail/en/c/1619514/> | maize harvest, handling, and field-to-conditioning context |
| `fao-maize-drying-handling` | handbook | FAO, Agricultural engineering in development: Grain and post-harvest systems, <https://www.fao.org/4/t0522e/T0522E03.htm> | drying, moisture, cleaning, conditioning, and process hand-off |
