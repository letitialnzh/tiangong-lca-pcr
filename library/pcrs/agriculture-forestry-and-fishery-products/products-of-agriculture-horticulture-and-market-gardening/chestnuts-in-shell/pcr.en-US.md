...
| covered_products | Whole chestnut fruit with husk and shell retained, and dehusked chestnuts with the hard shell retained when the extended gate is declared |
| excluded_products | Shelled or split chestnuts; chestnut kernel, chestnut oil, roasted or otherwise processed chestnut products, shell products, and post-gate logistics or retail products |
| representative_product | Mature or declared-maturity chestnuts in shell with cultivar, husk status, moisture or freshness state, grade, accepted mass, reject mass, and gate declared |
| production_route | Perennial chestnut-tree establishment and management, harvest and collection, optional dehusking while retaining the shell, primary conditioning, grading, optional storage, and declared hand-off |
| market_state | Fresh whole fruit at agricultural-field gate or dehusked/conditioned in-shell lot at a declared extended gate, with crop year, geography, maturity, husk status, shell condition, grade, and fate records |

## 3. Reference Flow

| Field | Value |
| --- | --- |
| What | Chestnuts in shell at the declared gate, with the hard shell retained and husk status declared |
| How much | 1,000 kg net accepted chestnuts in shell |
...
| reference_flow_link | Accepted output from `chestnut_harvest_and_collection` for the fresh whole-fruit state, or from `chestnut_primary_conditioning_and_grading` or `chestnut_storage_and_gate_handoff` when an extended gate is declared |

| Field | Value |
| --- | --- |
| Reference amount | 1,000 kg |
| Reference product flow | Chestnut `3351a3d2-3367-4a2f-8cdb-9b025fdd9989` |
...
The required qualifiers must be declared in dataset metadata, process notes, the reference-flow comment, product description, or an equivalent data-package field. Net product mass excludes container tare. The platform identity is the fresh agricultural-field gate product flow; dehusking, grading, drying, or storage is an extended gate and must retain that qualifier rather than being represented as a new product category. A shelled or split chestnut must not be used as the reference product for this PCR.

## 4. Measurement and Unit Rules

| rule_id | Applies to | Required property | Required unit | Rule |
| --- | --- | --- | --- | --- |
| `reference_mass` | accepted reference product | Mass `93a60a56-a3c8-11da-a746-0800200b9a66` | kg | Express the reference as net accepted chestnuts in shell at the declared gate and exclude container tare. |
...
| `moisture_and_freshness_basis` | harvested, conditioned, stored, and accepted chestnuts | Mass and moisture or freshness descriptor | kg and declared fraction or class | Preserve as-received mass and moisture or freshness class. Convert between states only with measured mass and a transparent water or dry-matter equation. |
...
The default foreground boundary runs from the declared tree-orchard starting condition through perennial tree management, harvest and collection, and the fresh agricultural-field gate hand-off of accepted chestnuts in shell. An extended route additionally includes dehusking that leaves the hard shell intact, cleaning, grading, storage, and the declared post-harvest gate. Upstream production of planting material, fertilizers, crop-protection products, supplied irrigation water, energy carriers, machinery services, and packaging uses representative background datasets. Shell removal, splitting, kernel processing, oil extraction, roasted or otherwise processed chestnut products, retail, post-gate transport, and post-gate storage are outside the default boundary.
...
| --- | --- | --- | --- |
| `boundary_tree_to_farm_gate` | all conforming datasets | Include perennial tree establishment, immature years, productive management, harvest, collection, and the declared fresh agricultural-field gate hand-off of accepted chestnuts in shell. | `fao-chestnut-sustainable-production-2023`; `iso-14044-2006` |
| `boundary_dehusking_extension` | extended post-harvest route | Include dehusking, cleaning, grading, and storage only when the declared dataset gate extends beyond the fresh whole-fruit agricultural-field gate. Report the extended gate and do not relabel it as the platform fresh/unprocessed state. | `fao-chestnut-postharvest-operations`; `codex-chestnut-hygiene-cxc-4-1971` |
| `boundary_perennial_tree_disclosure` | establishment and replacement | Allocate establishment, immature years, replacement trees, and removal over the declared tree life and accepted-output basis; disclose life, non-bearing years, intercropping, and replacement events. | `fao-chestnut-sustainable-production-2023`; `iso-14044-2006` |
| `boundary_no_shell_removal` | in-shell product | Retain the hard shell in the product boundary. Exclude hatcheting or shell removal, splitting, kernel processing, oil extraction, roasted or otherwise processed chestnut products, and shell processing. | `codex-chestnut-hygiene-cxc-4-1971`; `fao-chestnut-postharvest-operations` |
| `boundary_quality_and_fate` | all outputs | Separate accepted in-shell chestnuts, husk, downgraded lots, rejected fruit, foreign material, dust, water loss, storage loss, and unexplained loss by measured mass and declared fate. | `fao-chestnut-postharvest-operations`; `mass-balance-identity` |
...
| --- | --- | --- | --- | --- | --- |
| `chestnut_orchard_establishment_and_management` | Chestnut tree establishment and management | required |  | foreground | tree-year inputs and accepted output basis |
| `chestnut_harvest_and_collection` | Chestnut harvest and collection | required |  | foreground | collected whole fruit before optional dehusking |
| `chestnut_primary_conditioning_and_grading` | Chestnut dehusking, material treatment, primary conditioning, and grading | conditional | include when dehusking, cleaning, grading, or an extended gate is part of the foreground package | primary conditioning and stabilization | received lot and grade outputs |
| `chestnut_storage_and_gate_handoff` | Chestnut storage and gate hand-off | conditional | include when storage or a post-harvest gate is part of the declared foreground boundary | storage and delivery hand-off | 1,000 kg declared gate output |

### Process: Chestnut Tree Establishment and Management (`chestnut_orchard_establishment_and_management`)

#### Inputs

##### Product flows

###### Chestnut planting and replacement material (`chestnut_planting_material`)
...
- Selected flow: chestnut nursery plants and replacement planting material
...
- Normalization basis: per 1,000 kg accepted chestnuts in shell
- Basis kind: Reference flow (`reference_flow`)
- Evidence kind: Calculated from collection (`calculated_from_collection`)
- Collection protocol: `cp_chestnut_orchard_lifecycle_records`
...
###### Agricultural nutrient and fertilizer inputs (`chestnut_orchard_establishment_and_management_agricultural_nutrient_inputs`)
...
- Collection protocol: `cp_chestnut_orchard_input_records`
...
###### Supplied irrigation water (`chestnut_irrigation_water`)
...
- Selected flow: irrigation water supplied to chestnut trees
...
- Basis kind: Reference flow (`reference_flow`)
- Evidence kind: Collected record (`collected_record`)
- Collection protocol: `cp_chestnut_irrigation_records`
...
###### Field fuel and electricity (`chestnut_field_energy`)
...
- Basis kind: Reference flow (`reference_flow`)
- Evidence kind: Collected record (`collected_record`)
- Collection protocol: `cp_chestnut_energy_records`
...
###### Orchard pruning and fallen-fruit residue (`chestnut_orchard_residue`)
...
- Selected flow: chestnut orchard residue and removed biomass
...
###### Managed-soil and crop-protection emissions (`chestnut_field_emissions`)
...
- Basis kind: Reference flow (`reference_flow`)
- Evidence kind: Calculated from collection (`calculated_from_collection`)
- Collection protocol: `cp_chestnut_emission_records`
- Sources: `ipcc-2019-afolu`
...
### Process: Chestnut Harvest and Collection (`chestnut_harvest_and_collection`)

#### Inputs

##### Product flows

###### Whole chestnuts available for collection (`chestnut_harvest_available`)
...
- Selected flow: Chestnuts, in shell `3351a3d2-3367-4a2f-8cdb-9b025fdd9989`
- Binding: `fixed`
...
- Basis kind: Reference flow (`reference_flow`)
- Evidence kind: Collected record (`collected_record`)
- Collection protocol: `cp_chestnut_harvest_lot_records`
...
###### Harvest and collection energy (`chestnut_harvest_energy`)

Record climbing, cutting, collection, field transport, and handling energy by harvest round and equipment.

- Selected flow: fuel or electricity used for chestnut harvest and collection
- Binding: `parameterized`
- Flow Set: `flow-set.energy-supply`
- Flow Set version: `0.2.0`
- Flow property / unit: Mass, volume, or energy / kg, L, MJ, or kWh
...
### Process: Chestnut Dehusking, Primary Conditioning, and Grading (`chestnut_primary_conditioning_and_grading`)

This process is conditional. Dehusking removes the fibrous husk while leaving the hard shell intact; shell removal, splitting, and kernel or chestnut-water recovery are outside this PCR.
...
###### Whole chestnuts received for conditioning (`chestnut_conditioning_input`)
...
- Evidence kind: Collected record (`collected_record`)
- Collection protocol: `cp_chestnut_dehusking_conditioning_records`
...
###### Dehusking and grading energy (`chestnut_conditioning_energy`)
...
###### Conditioning water or cleaning service (`chestnut_conditioning_water`)
...
###### Accepted chestnuts in shell (`chestnut_conditioned_accepted_output`)

Record accepted whole or dehusked chestnuts with shell retained, grade, maturity, husk status, and declared gate.
The accepted chestnut lot is the intended output of this conditioning process.
...
- Basis kind: Reference flow (`reference_flow`)
- Evidence kind: Collected record (`collected_record`)
- Collection protocol: `cp_chestnut_grade_reject_records`
...
###### Marketed chestnut husk co-product (`chestnut_husk_coproduct`)
...
- Selected flow: chestnut husk and fibrous biomass
- Flow property / unit: Mass / kg
...
- Basis kind: Process output (`process_output`)
- Evidence kind: Collected record (`collected_record`)
- Collection protocol: `cp_chestnut_husk_destination_records`
...
###### Chestnut husk and removed biomass (`chestnut_husk_output_or_residue`)
...
- Selected flow: chestnut husk and fibrous residue
- Flow property / unit: Mass / kg
...
###### Downgraded and rejected chestnuts (`chestnut_grade_rejects`)

Record cracked, moldy, insect-damaged, immature, over-mature, contaminated, or otherwise off-spec chestnuts, with rework, recovery, treatment, or disposal destination.

- Selected flow: rejected or downgraded chestnuts in shell
- Flow property / unit: Mass / kg
...
###### Conditioning wastewater, dust, and wash solids (`chestnut_conditioning_residuals`)
...
### Process: Chestnut Storage and Gate Hand-off (`chestnut_storage_and_gate_handoff`)
...
###### Chestnuts received for storage (`chestnut_storage_input`)
...
- Basis kind: Reference flow (`reference_flow`)
- Evidence kind: Collected record (`collected_record`)
- Collection protocol: `cp_chestnut_storage_records`
...
###### Storage packaging (`chestnut_storage_packaging`)
...
###### Accepted chestnuts at declared gate (`chestnut_gate_output`)
...
| --- | --- | --- | --- |
| `quality_01` | flow identity | Use the verified `Chestnuts, in shell` UUID with Mass and kg. Preserve the fresh agricultural-field gate or explicitly extended-gate qualifier and do not substitute shelled, split, kernel, or water identities. | platform identity record; dataset metadata |
...
| rule_id | applies_to | Rule | source_ids |
| --- | --- | --- | --- |
| `validation_01` | reference flow | Reference product is the verified `Chestnuts, in shell` UUID with Mass and kg. The dataset declares whole-fruit or extended dehusked gate state and does not substitute a shelled or split chestnut. | `codex-chestnut-hygiene-cxc-4-1971` |
| `validation_02` | process map | Every required process is present; conditional dehusking, cleaning, grading, and storage processes are present when the declared gate or route requires them. | `fao-chestnut-postharvest-operations` |
| `validation_03` | perennial production | Tree establishment, immature years, productive phase, replacement, intercropping attribution, and removal are linked to declared crop years and are not double-counted. | `fao-chestnut-sustainable-production-2023`; `iso-14044-2006` |
| `validation_04` | harvest and collection | Harvested, collected, uncollected, damaged, residue, and accepted chestnut masses are mutually reconciled; maturity and hand-off state are declared. | `mass-balance-identity` |
| `validation_05` | dehusking and quality | Whole-fruit input, husk, accepted in-shell product, grade, downgraded/rework, reject, foreign material, water loss, and residuals are mutually exclusive and mass-balanced. | `mass-balance-identity`; `fao-chestnut-postharvest-operations` |
| `validation_06` | storage and gate | Opening stock, receipts, dispatches, closing stock, storage damage, product state, storage duration, and gate output reconcile. Storage losses are excluded from accepted output. | `mass-balance-identity`; `fao-chestnut-postharvest-operations` |
...
| `validation_08` | evidence and disclosure | All important quantitative flows have a foreground collection rule and candidate range or method constraint; estimates, unresolved identities, gate state, husk fate, and evidence limitations are disclosed. | `fao-chestnut-postharvest-operations`; `mass-balance-identity` |
...
| Field | Value |
| --- | --- |
| dataset_role | `secondary_dataset` for a reviewed chestnut-in-shell foreground dataset; may be promoted to `background_dataset` only after the required review and publication gates |
| downstream_use | Construction and validation of process and lifecyclemodel projections for chestnuts in shell at the declared fresh agricultural-field gate or extended dehusked, conditioned, or storage gate |
...
| excluded_use | Shell removal, splitting, kernel processing, oil extraction, roasted or otherwise processed chestnut products, shell products, retail products, consumer use, post-gate logistics, and unreported product categories |
| required_metadata | reference UUID; Mass property and Units of mass unit group; CPC 3.0 `01373`; cultivar; geography; tree phase and age; crop year; irrigation; maturity; whole-fruit or dehusked state; husk fate; shell and grade quality; accepted, rejected, residue, loss, and storage records |
...
| --- | --- | --- | --- |
| `fao-chestnut-sustainable-production-2023` | official_guidance | FAO family farming, Sustainable production guide of chestnut, COLEAD, https://www.fao.org/family-farming/detail/en/c/1755586/ | climate, soil, nutrition, tree establishment, management, harvest, and post-harvest route context |
| `fao-chestnut-postharvest-operations` | handbook | FAO INPhO, Chestnut: Post-harvest Operations, https://www.fao.org/fileadmin/user_upload/inpho/docs/Post_Harvest_Compendium_-_Chestnut.pdf | fruit structure, dehusking, post-harvest operations, product states, storage, and downstream exclusions |
| `codex-chestnut-hygiene-cxc-4-1971` | standard | Codex Alimentarius, Code of Hygienic Practice for Desiccated Chestnut CXC 4-1971, https://www.fao.org/fao-who-codexalimentarius/sh-proxy/fr/?lnk=1&url=https%253A%252F%252Fworkspace.fao.org%252Fsites%252Fcodex%252FStandards%252FCXC%2B4-1971%252FCXC_004e.pdf | definitions of husk and shell removal, hygienic handling, and boundary distinctions for downstream chestnut processing |
