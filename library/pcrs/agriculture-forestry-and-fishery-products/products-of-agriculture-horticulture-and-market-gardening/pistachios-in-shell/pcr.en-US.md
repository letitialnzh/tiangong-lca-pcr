...
This PCR guides a foreground data package for pistachios produced in managed perennial pistachio-tree systems and transferred as pistachios in shell at a declared farm, primary-conditioning, storage, or hand-off gate. It covers tree establishment and replacement, productive-tree management, harvest and collection, optional dehusking that leaves the hard shell intact, cleaning, grading, storage, and the declared gate hand-off.

The canonical reference-flow identity is the platform record `Pistachios, in shell`, classified as CPC 3.0 `01375`, with the platform qualifier `Production mix, at plant; Fresh pistachios picked`. A dataset must disclose whether the declared gate is the whole fruit at the fresh plant gate, a dehusked pistachio with shell retained, or an extended conditioned or stored lot. These states must not be silently mixed.

This PCR applies to conventional, organic, rain-fed, irrigated, mixed-crop, and smallholder or estate routes when the cultivar or cultivar group, geography, tree age, crop year, harvest maturity, husk status, shell condition, moisture basis, quality state, and gate are disclosed. It excludes nursery operations except as upstream inputs, shell removal, splitting or hatcheting, kernel processing, oil extraction, roasted or otherwise processed pistachio products, shell processing, retail packaging, consumer use, and transport or storage after the declared gate. It does not select an `Other` or n.e.c. product category.

## 2. Product Category Identity

| Field | Value |
| --- | --- |
| canonical_pcr_id | `pcr.agriculture-forestry-and-fishery-products.products-of-agriculture-horticulture-and-market-gardening.pistachios-in-shell` |
| classification_refs | CPC 3.0 `01375`, Pistachios, in shell |
| covered_products | Whole pistachio fruit with husk and shell retained, and dehusked pistachios with the hard shell retained when the extended gate is declared |
| excluded_products | Shelled or split pistachios; pistachio kernel, pistachio oil, roasted or otherwise processed pistachio products, shell products, and post-gate logistics or retail products |
| representative_product | Mature or declared-maturity pistachios in shell with cultivar, husk status, moisture or freshness state, grade, accepted mass, reject mass, and gate declared |
| production_route | Perennial pistachio-tree establishment and management, harvest and collection, optional dehusking while retaining the shell, primary conditioning, grading, optional storage, and declared hand-off |
| market_state | Fresh whole fruit at plant gate or dehusked/conditioned in-shell lot at a declared extended gate, with crop year, geography, maturity, husk status, shell condition, grade, and fate records |

## 3. Reference Flow

| Field | Value |
| --- | --- |
| What | Pistachios in shell at the declared gate, with the hard shell retained and husk status declared |
| How much | 1,000 kg net accepted pistachios in shell |
...
| reference_flow_link | Accepted output from `pistachio_harvest_and_collection` for the fresh whole-fruit state, or from `pistachio_primary_conditioning_and_grading` or `pistachio_storage_and_gate_handoff` when an extended gate is declared |

| Field | Value |
| --- | --- |
| Reference amount | 1,000 kg |
| Reference product flow | Pistachio `8ccf9ca8-1c99-4c68-b8c5-fae73b0215b8` |
...
The required qualifiers must be declared in dataset metadata, process notes, the reference-flow comment, product description, or an equivalent data-package field. Net product mass excludes container tare. The platform identity is the fresh plant-gate product flow; dehusking, grading, drying, or storage is an extended gate and must retain that qualifier rather than being represented as a new product category. A shelled or split pistachio must not be used as the reference product for this PCR.

## 4. Measurement and Unit Rules

| rule_id | Applies to | Required property | Required unit | Rule |
| --- | --- | --- | --- | --- |
| `reference_mass` | accepted reference product | Mass `93a60a56-a3c8-11da-a746-0800200b9a66` | kg | Express the reference as net accepted pistachios in shell at the declared gate and exclude container tare. |
...
| `moisture_and_freshness_basis` | harvested, conditioned, stored, and accepted pistachios | Mass and moisture or freshness descriptor | kg and declared fraction or class | Preserve as-received mass and moisture or freshness class. Convert between states only with measured mass and a transparent water or dry-matter equation. |
...
The default foreground boundary runs from the declared tree-orchard starting condition through perennial tree management, harvest and collection, and the fresh plant-gate hand-off of accepted pistachios in shell. An extended route additionally includes dehusking that leaves the hard shell intact, cleaning, grading, storage, and the declared post-harvest gate. Upstream production of planting material, fertilizers, crop-protection products, supplied irrigation water, energy carriers, machinery services, and packaging uses representative background datasets. Shell removal, splitting, kernel processing, oil extraction, roasted or otherwise processed pistachio products, retail, post-gate transport, and post-gate storage are outside the default boundary.
...
| starting_condition_role | start of managed perennial pistachio production and tree-asset accounting |
| product_classification_scope | CPC 3.0 `01375`, Pistachios, in shell; shelled, split, kernel, oil, shell, and downstream processed pistachio products are outside this PCR |
| recursive_input_rule | Same-category planting material or retained pistachios crossing the boundary are recorded as upstream inputs and are not recursively expanded into another in-shell product output. Internal hand-offs between orchard, harvest, conditioning, grading, and storage are process links and are not additional market outputs. |
...
| --- | --- | --- | --- |
| `boundary_tree_to_farm_gate` | all conforming datasets | Include perennial tree establishment, immature years, productive management, harvest, collection, and the declared fresh plant-gate hand-off of accepted pistachios in shell. | `fao-pistachio-sustainable-production-2023`; `iso-14044-2006` |
| `boundary_dehusking_extension` | extended post-harvest route | Include dehusking, cleaning, grading, and storage only when the declared dataset gate extends beyond the fresh whole-fruit plant gate. Report the extended gate and do not relabel it as the platform fresh/unprocessed state. | `fao-pistachio-postharvest-operations`; `codex-pistachio-hygiene-cxc-4-1971` |
| `boundary_perennial_tree_disclosure` | establishment and replacement | Allocate establishment, immature years, replacement trees, and removal over the declared tree life and accepted-output basis; disclose life, non-bearing years, intercropping, and replacement events. | `fao-pistachio-sustainable-production-2023`; `iso-14044-2006` |
| `boundary_no_shell_removal` | in-shell product | Retain the hard shell in the product boundary. Exclude hatcheting or shell removal, splitting, kernel processing, oil extraction, roasted or otherwise processed pistachio products, and shell processing. | `codex-pistachio-hygiene-cxc-4-1971`; `fao-pistachio-postharvest-operations` |
| `boundary_quality_and_fate` | all outputs | Separate accepted in-shell pistachios, husk, downgraded lots, rejected fruit, foreign material, dust, water loss, storage loss, and unexplained loss by measured mass and declared fate. | `fao-pistachio-postharvest-operations`; `mass-balance-identity` |
...
| --- | --- | --- | --- | --- | --- |
| `pistachio_orchard_establishment_and_management` | Pistachio tree establishment and management | required |  | foreground | tree-year inputs and accepted output basis |
| `pistachio_harvest_and_collection` | Pistachio harvest and collection | required |  | foreground | collected whole fruit before optional dehusking |
| `pistachio_primary_conditioning_and_grading` | Pistachio dehusking, material treatment, primary conditioning, and grading | conditional | include when dehusking, cleaning, grading, or an extended gate is part of the foreground package | primary conditioning and stabilization | received lot and grade outputs |
| `pistachio_storage_and_gate_handoff` | Pistachio storage and gate hand-off | conditional | include when storage or a post-harvest gate is part of the declared foreground boundary | storage and delivery hand-off | 1,000 kg declared gate output |

### Process: Pistachio Tree Establishment and Management (`pistachio_orchard_establishment_and_management`)

#### Inputs

##### Product flows

###### Pistachio planting and replacement material (`pistachio_planting_material`)
...
- Selected flow: pistachio nursery plants and replacement planting material
...
- Normalization basis: per 1,000 kg accepted pistachios in shell
- Basis kind: Reference flow (`reference_flow`)
- Evidence kind: Calculated from collection (`calculated_from_collection`)
- Collection protocol: `cp_pistachio_orchard_lifecycle_records`
...
###### Agricultural nutrient and fertilizer inputs (`pistachio_orchard_establishment_and_management_agricultural_nutrient_inputs`)
...
- Collection protocol: `cp_pistachio_orchard_input_records`
...
###### Supplied irrigation water (`pistachio_irrigation_water`)
...
- Selected flow: irrigation water supplied to pistachio trees
...
- Basis kind: Reference flow (`reference_flow`)
- Evidence kind: Collected record (`collected_record`)
- Collection protocol: `cp_pistachio_irrigation_records`
...
###### Field fuel and electricity (`pistachio_field_energy`)
...
- Basis kind: Reference flow (`reference_flow`)
- Evidence kind: Collected record (`collected_record`)
- Collection protocol: `cp_pistachio_energy_records`
...
###### Orchard pruning and fallen-fruit residue (`pistachio_orchard_residue`)
...
- Selected flow: pistachio orchard residue and removed biomass
...
###### Managed-soil and crop-protection emissions (`pistachio_field_emissions`)
...
- Basis kind: Reference flow (`reference_flow`)
- Evidence kind: Calculated from collection (`calculated_from_collection`)
- Collection protocol: `cp_pistachio_emission_records`
- Sources: `ipcc-2019-afolu`
...
### Process: Pistachio Harvest and Collection (`pistachio_harvest_and_collection`)

#### Inputs

##### Product flows

###### Whole pistachios available for collection (`pistachio_harvest_available`)
...
- Selected flow: Pistachios, in shell `8ccf9ca8-1c99-4c68-b8c5-fae73b0215b8`
- Binding: `fixed`
...
- Basis kind: Reference flow (`reference_flow`)
- Evidence kind: Collected record (`collected_record`)
- Collection protocol: `cp_pistachio_harvest_lot_records`
...
###### Harvest and collection energy (`pistachio_harvest_energy`)

Record climbing, cutting, collection, field transport, and handling energy by harvest round and equipment.

- Selected flow: fuel or electricity used for pistachio harvest and collection
- Binding: `parameterized`
- Flow Set: `flow-set.energy-supply`
- Flow Set version: `0.2.0`
- Flow property / unit: Mass, volume, or energy / kg, L, MJ, or kWh
...
### Process: Pistachio Dehusking, Primary Conditioning, and Grading (`pistachio_primary_conditioning_and_grading`)

This process is conditional. Dehusking removes the fibrous husk while leaving the hard shell intact; shell removal, splitting, and kernel or pistachio-water recovery are outside this PCR.
...
###### Whole pistachios received for conditioning (`pistachio_conditioning_input`)
...
- Evidence kind: Collected record (`collected_record`)
- Collection protocol: `cp_pistachio_dehusking_conditioning_records`
...
###### Dehusking and grading energy (`pistachio_conditioning_energy`)
...
###### Conditioning water or cleaning service (`pistachio_conditioning_water`)
...
###### Accepted pistachios in shell (`pistachio_conditioned_accepted_output`)

Record accepted whole or dehusked pistachios with shell retained, grade, maturity, husk status, and declared gate.
The accepted pistachio lot is the intended output of this conditioning process.
...
- Basis kind: Reference flow (`reference_flow`)
- Evidence kind: Collected record (`collected_record`)
- Collection protocol: `cp_pistachio_grade_reject_records`
...
###### Marketed pistachio husk co-product (`pistachio_husk_coproduct`)
...
- Selected flow: pistachio husk and fibrous biomass
- Flow property / unit: Mass / kg
...
- Basis kind: Process output (`process_output`)
- Evidence kind: Collected record (`collected_record`)
- Collection protocol: `cp_pistachio_husk_destination_records`
...
###### Pistachio husk and removed biomass (`pistachio_husk_output_or_residue`)
...
- Selected flow: pistachio husk and fibrous residue
- Flow property / unit: Mass / kg
...
###### Downgraded and rejected pistachios (`pistachio_grade_rejects`)

Record cracked, moldy, insect-damaged, immature, over-mature, contaminated, or otherwise off-spec pistachios, with rework, recovery, treatment, or disposal destination.

- Selected flow: rejected or downgraded pistachios in shell
- Flow property / unit: Mass / kg
...
###### Conditioning wastewater, dust, and wash solids (`pistachio_conditioning_residuals`)
...
### Process: Pistachio Storage and Gate Hand-off (`pistachio_storage_and_gate_handoff`)
...
###### Pistachios received for storage (`pistachio_storage_input`)
...
- Basis kind: Reference flow (`reference_flow`)
- Evidence kind: Collected record (`collected_record`)
- Collection protocol: `cp_pistachio_storage_records`
...
###### Storage energy (`pistachio_storage_energy`)

Record electricity, fuel, cooling, ventilation, or other storage energy by facility and storage period.

- Selected flow: fuel or electricity used for pistachio storage
- Binding: `parameterized`
- Flow Set: `flow-set.energy-supply`
- Flow Set version: `0.2.0`
- Flow property / unit: Mass, volume, or energy / kg, L, MJ, or kWh
...
###### Storage packaging (`pistachio_storage_packaging`)
...
###### Accepted pistachios at declared gate (`pistachio_gate_output`)
...
| --- | --- | --- | --- |
| `quality_01` | flow identity | Use the verified `Pistachios, in shell` UUID with Mass and kg. Preserve the fresh plant-gate or explicitly extended-gate qualifier and do not substitute shelled, split, kernel, or water identities. | platform identity record; dataset metadata |
...
| rule_id | applies_to | Rule | source_ids |
| --- | --- | --- | --- |
| `validation_01` | reference flow | Reference product is the verified `Pistachios, in shell` UUID with Mass and kg. The dataset declares whole-fruit or extended dehusked gate state and does not substitute a shelled or split pistachio. | `codex-pistachio-hygiene-cxc-4-1971` |
| `validation_02` | process map | Every required process is present; conditional dehusking, cleaning, grading, and storage processes are present when the declared gate or route requires them. | `fao-pistachio-postharvest-operations` |
| `validation_03` | perennial production | Tree establishment, immature years, productive phase, replacement, intercropping attribution, and removal are linked to declared crop years and are not double-counted. | `fao-pistachio-sustainable-production-2023`; `iso-14044-2006` |
| `validation_04` | harvest and collection | Harvested, collected, uncollected, damaged, residue, and accepted pistachio masses are mutually reconciled; maturity and hand-off state are declared. | `mass-balance-identity` |
| `validation_05` | dehusking and quality | Whole-fruit input, husk, accepted in-shell product, grade, downgraded/rework, reject, foreign material, water loss, and residuals are mutually exclusive and mass-balanced. | `mass-balance-identity`; `fao-pistachio-postharvest-operations` |
| `validation_06` | storage and gate | Opening stock, receipts, dispatches, closing stock, storage damage, product state, storage duration, and gate output reconcile. Storage losses are excluded from accepted output. | `mass-balance-identity`; `fao-pistachio-postharvest-operations` |
...
| `validation_08` | evidence and disclosure | All important quantitative flows have a foreground collection rule and candidate range or method constraint; estimates, unresolved identities, gate state, husk fate, and evidence limitations are disclosed. | `fao-pistachio-postharvest-operations`; `mass-balance-identity` |
...
| Field | Value |
| --- | --- |
| dataset_role | `secondary_dataset` for a reviewed pistachio-in-shell foreground dataset; may be promoted to `background_dataset` only after the required review and publication gates |
| downstream_use | Construction and validation of process and lifecyclemodel projections for pistachios in shell at the declared fresh plant-gate or extended dehusked, conditioned, or storage gate |
...
| excluded_use | Shell removal, splitting, kernel processing, oil extraction, roasted or otherwise processed pistachio products, shell products, retail products, consumer use, post-gate logistics, and unreported product categories |
| required_metadata | reference UUID; Mass property and Units of mass unit group; CPC 3.0 `01375`; cultivar; geography; tree phase and age; crop year; irrigation; maturity; whole-fruit or dehusked state; husk fate; shell and grade quality; accepted, rejected, residue, loss, and storage records |
...
| --- | --- | --- | --- |
| `fao-pistachio-sustainable-production-2023` | official_guidance | FAO family farming, Sustainable production guide of pistachio, COLEAD, https://www.fao.org/family-farming/detail/en/c/1755586/ | climate, soil, nutrition, tree establishment, management, harvest, and post-harvest route context |
| `fao-pistachio-postharvest-operations` | handbook | FAO INPhO, Pistachio: Post-harvest Operations, https://www.fao.org/fileadmin/user_upload/inpho/docs/Post_Harvest_Compendium_-_Pistachio.pdf | fruit structure, dehusking, post-harvest operations, product states, storage, and downstream exclusions |
| `codex-pistachio-hygiene-cxc-4-1971` | standard | Codex Alimentarius, Code of Hygienic Practice for Desiccated Pistachio CXC 4-1971, https://www.fao.org/fao-who-codexalimentarius/sh-proxy/fr/?lnk=1&url=https%253A%252F%252Fworkspace.fao.org%252Fsites%252Fcodex%252FStandards%252FCXC%2B4-1971%252FCXC_004e.pdf | definitions of husk and shell removal, hygienic handling, and boundary distinctions for downstream pistachio processing |
