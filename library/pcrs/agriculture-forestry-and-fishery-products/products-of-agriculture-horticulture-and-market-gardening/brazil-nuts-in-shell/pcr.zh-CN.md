...
本 PCR 适用于常规、综合、有机、雨养和灌溉路线，但必须披露品种或品种组、地理位置、巴西坚果园年龄、作物年度、采收方式、干燥路线、水分基准、质量状态和交接点。不包括苗圃生产、巴西坚果荚果开启取仁、烘烤、蒸煮、去皮、巴西坚果壳液回收、仁分级、食用坚果加工、零售包装、消费者使用以及声明交接点后的运输或储存。本 PCR 不选择“other”或 n.e.c. 产品类别。
...
| reference_flow_link | 鲜品农场交接点参考状态来自 `brazil-nut_harvest_and_collection` 的合格输出；声明延伸后处理交接点时来自 `brazil-nut_storage_and_gate_handoff` |
...
默认前景边界从声明的巴西坚果园起始条件开始，经过多年生巴西坚果园管理、采收与收集、巴西坚果木质荚果分离，到合格带壳巴西坚果在鲜品农场交接点的交接。延伸调制路线另纳入日晒或机械干燥、清理、带壳质量分选、储存和声明的后处理交接点。种植材料、肥料、植保产品、灌溉水、能源载体、机械服务和包装的上游生产使用有代表性的背景数据集。荚果开启取仁、巴西坚果壳液回收、仁加工、零售、交接点后运输和储存不在默认边界内。
...
| --- | --- | --- | --- |
| `boundary_tree_to_farm_gate` | 所有符合条件的数据集 | 纳入多年生巴西坚果园建立、未结果年份、结果期管理、采收、收集、巴西坚果木质荚果分离和声明鲜品农场交接点交接。 | `fao-brazil-nut-india-production-1998`; `iso-14044-2006` |
| `boundary_dehusking_extension` | 延伸后处理路线 | 只有声明交接点超出鲜品农场交接点，或这些作业属于前景数据包时，才纳入干燥、清理、带壳分级和储存；报告延伸交接点，不得重新标注为平台鲜品未加工状态。 | `fao-brazil-nut-postharvest-2020`; `fao-brazil-nut-minor-oil-crops` |
| `boundary_perennial_tree_disclosure` | 建立和补植 | 将建立、未结果年份、补植树和清除按巴西坚果园寿命及合格产出基准分配，并披露寿命、未结果年份和补植事件。 | `fao-brazil-nut-india-production-1998`; `iso-14044-2006` |
| `boundary_no_shell_removal` | 带壳产品 | 硬壳属于产品边界；排除荚果开启取仁、巴西坚果壳液回收、仁分离、烘烤、蒸煮、去皮和仁分级，因为它们属于下游去壳仁路线。 | `fda-brazil-nut-nut-methods`; `fao-brazil-nut-minor-oil-crops` |
...
### 过程：巴西坚果园建立和管理（`brazil-nut_forest_establishment_and_management`）

#### 输入

##### 产品流

###### 巴西坚果种植和补植材料（`brazil-nut_planting_material`）

按地块和事件记录苗木、嫁接材料、补植树、支杆以及归属的建园材料，并将其按巴西坚果园寿命和合格带壳产出年度化。
...
- 归一化基准：每 1,000 kg 合格带壳巴西坚果
- 基准类型：参考流（`reference_flow`）
- 证据类型：根据采集计算（`calculated_from_collection`）
- 采集协议：`cp_brazil-nut_forest_lifecycle_records`
- 数量范围：暂定种植材料筛选估计
  - 范围角色：默认估计（`default_estimate`）
  - 下限：0.1
  - 上限：150
  - 单位：每 1,000 kg 参考产品的 kg 或种植单位
  - 基准：巴西坚果园建立和补植的宽范围首轮估计
  - 基准类型：参考流（`reference_flow`）
  - 证据类型：推理估算（`reasoned_estimate`）

###### 农业养分与肥料投入（`brazil-nut_forest_establishment_and_management_agricultural_nutrient_inputs`）
...
- 采集协议：`cp_brazil-nut_forest_input_records`
- 来源：`ipcc-2019-afolu`
- 数量范围：暂定养分投入筛选范围
  ...
  - 基准：年度巴西坚果园养分和改良剂宽范围投入
  - 基准类型：参考流（`reference_flow`）
  - 证据类型：推理估算（`reasoned_estimate`）

###### 供应的灌溉水（`brazil-nut_irrigation_water`）
...
- 基准类型：参考流（`reference_flow`）
- 证据类型：采集记录（`collected_record`）
- 采集协议：`cp_brazil-nut_irrigation_records`
- 数量范围：暂定灌溉筛选范围
  - 范围角色：QA 校验范围（`qa_guardrail`）
  - 下限：0
  - 上限：2,500
  - 单位：每 1,000 kg 参考产品的 m3
  - 基准：雨养至灌溉巴西坚果园的宽范围用水区间
  - 基准类型：参考流（`reference_flow`）
  - 证据类型：推理估算（`reasoned_estimate`）

###### 巴西坚果园燃料和电力（`brazil-nut_field_energy`）
...
- 基准类型：参考流（`reference_flow`）
- 证据类型：采集记录（`collected_record`）
- 采集协议：`cp_brazil-nut_energy_records`
- 数量范围：暂定巴西坚果园能源筛选范围
  - 范围角色：QA 校验范围（`qa_guardrail`）
  - 下限：0
  - 上限：500
...
###### 植物保护产品（`brazil-nut_crop_protection`）
...
###### 巴西坚果园修剪和补植残余（`brazil-nut_forest_residue`）
...
- 数量范围：暂定巴西坚果园残余筛选范围
  ...
  - 基准：修剪、补植和巴西坚果园地面残余的宽范围
  - 基准类型：作物周期（`crop_cycle`）
  - 证据类型：推理估算（`reasoned_estimate`）

##### 初级流

###### 巴西坚果园土地占用（`brazil-nut_land_occupation`）

按地块和作物年度记录种植面积与巴西坚果园占用；土地转化单独记录，不能由占用量推断。

- 选定流：巴西坚果园农业土地占用
- 流属性/单位：面积时间 / ha·巴西坚果园年
...
- 数量范围：暂定土地占用筛选范围
  ...
  - 下限：0.1
  - 上限：6
  - 单位：每 1,000 kg 参考产品的 ha·巴西坚果园年
  - 基准：多年生坚果作物巴西坚果园面积宽范围
  - 基准类型：参考流（`reference_flow`）
  - 证据类型：推理估算（`reasoned_estimate`）
...
###### 管理土壤和作物直接排放（`brazil-nut_field_emissions`）
...
- 基准类型：参考流（`reference_flow`）
- 证据类型：根据采集计算（`calculated_from_collection`）
- 采集协议：`cp_brazil-nut_emission_records`
- 来源：`ipcc-2019-afolu`
- 数量范围：暂定直接排放筛选范围
  - 范围角色：QA 校验范围（`qa_guardrail`）
  - 下限：0
...
### 过程：巴西坚果采收、巴西坚果木质荚果分离和收集（`brazil-nut_harvest_and_collection`）

#### 输入

##### 产品流

###### 可采收的带壳巴西坚果作物（`brazil-nut_harvest_available`）

按地块和收集轮次记录可供采收的作物。平台参考身份只在鲜品、农场交接点、带壳限定下复用，不代表批次已经干燥或荚果开启。
...
- 数值来源模式：前景记录（`foreground_record`）
- 适用范围：场址特定（`site_specific`）
- 归一化基准：每 1,000 kg 合格鲜品农场交接点参考产品
- 基准类型：参考流（`reference_flow`）
- 证据类型：采集记录（`collected_record`）
- 采集协议：`cp_brazil-nut_harvest_lot_records`
- 数量范围：可采收质量 QA 校验范围
  - 范围角色：QA 校验（`qa_guardrail`）
  - 下限：900
  - 上限：1,400
  - 单位：每 1,000 kg 合格参考产品的 kg 鲜带壳巴西坚果
  - 基准：分选和收集损失前的同批次采收质量
  - 基准类型：参考流（`reference_flow`）
  - 证据类型：方法公式（`method_formula`）
  - 来源：`mass-balance-identity`

###### 采收和收集能源（`brazil-nut_harvest_energy`）

按采收批次记录震落、收集、巴西坚果木质荚果分离、装载和向下一节点转移所用燃料、电力和设备服务。
...
###### 未收集或田间损伤的巴西坚果（`brazil-nut_harvest_loss`）

记录留在巴西坚果园、收集前受损或从收集批次排除的成熟巴西坚果，并披露去向和水分基准。
...
###### 巴西坚果木质荚果残余或有意副产品（`brazil-nut_husk_output_or_residue`）

有意输出集合包括合格带壳等级批次，以及在有意收集时的巴西坚果木质荚果副产品；每个有意输出都必须有声明交接点。苹果被有意收集并交付给声明用户时，作为具有实测去向的有意副产品记录；未作为产品收集时，记录其质量和残余去向，不分配市场信用。

- 选定流：交付至声明去向的巴西坚果木质荚果或巴西坚果木质荚果残余
- 流属性/单位：质量 / kg 接收状态
...
- 基准类型：参考流（`reference_flow`）
- 证据类型：采集记录（`collected_record`）
- 采集协议：`cp_brazil-nut_husk_destination_records`
- 数量范围：巴西坚果木质荚果去向筛选范围
  - 范围角色：QA 校验范围（`qa_guardrail`）
  - 下限：0
  - 上限：2,000
  - 单位：每 1,000 kg 参考产品的 kg 接收状态苹果
  - 基准：苹果与巴西坚果质量宽范围；以实测批次替换
  - 基准类型：参考流（`reference_flow`）
  - 证据类型：推理估算（`reasoned_estimate`）

###### 采收和收集残余（`brazil-nut_harvest_residue`）

记录叶片、茎、土、石块和收集时移除的其他材料，并说明是否留在巴西坚果园、堆肥或离开前景边界。
...
### 过程：巴西坚果初级干燥、清理和带壳分级（`brazil-nut_primary_conditioning_and_grading`）

本过程为条件过程。当声明交接点超出平台鲜品农场交接点，或前景数据包包含干燥、清理和质量分选时纳入。本过程是独立的物料处理节点，具有接收带壳批次、调制和分级输出状态以及声明交接点；不荚果开启，也不产生去壳仁产品。

#### 输入

##### 产品流

###### 鲜收带壳巴西坚果批次（`brazil-nut_conditioning_input`）

使用已核实的平台鲜带壳批次身份，同时保留已收集、已分离巴西坚果木质荚果、调制前状态限定信息。
...
- 基准类型：参考流（`reference_flow`）
- 证据类型：采集记录（`collected_record`）
- 采集协议：`cp_brazil-nut_dehusking_conditioning_records`
- 数量范围：调制输入质量 QA 校验范围
  - 范围角色：QA 校验范围（`qa_guardrail`）
  - 下限：900
  - 上限：1,250
...
###### 干燥和清理用水或辅助服务（`brazil-nut_conditioning_water`）
...
###### 干燥和调制能源（`brazil-nut_conditioning_energy`）
...
###### 高等级合格带壳巴西坚果（`brazil-nut_premium_grade_output`）

记录作为有意输出的合格等级和交接；产品仍是带壳巴西坚果，荚果开启和仁分级不在本 PCR 内。
...
- 数值来源模式：前景记录（`foreground_record`）
- 适用范围：产品特定（`product_specific`）
- 归一化基准：每 1,000 kg 鲜品农场交接点参考产品
- 基准类型：参考流（`reference_flow`）
- 证据类型：采集记录（`collected_record`）
- 采集协议：`cp_brazil-nut_grade_reject_records`
- 数量范围：高等级输出筛选范围
  - 范围角色：QA 校验范围（`qa_guardrail`）
  - 下限：0
  - 上限：1,000
...
###### 标准等级合格带壳巴西坚果（`brazil-nut_standard_grade_output`）
...
###### 降级或可返工带壳批次（`brazil-nut_downgraded_lot`）

将可能销售或返工的规格外带壳巴西坚果与合格等级分开，并声明降级、返回清理或干燥、恢复至其他带壳去向或废弃路径。
...
###### 拒收巴西坚果和空壳（`brazil-nut_rejected_nuts`）

记录空壳、霉变、虫害、明显变质或其他拒收巴西坚果，保留拒收原因和去向；拒收质量不计入合格输出。
...
###### 异物和粉尘（`brazil-nut_foreign_material`）

记录清理过程中移除的土、石、叶、茎、不属于合格巴西坚果的壳片和粉尘；有意回收时记录去向，不得静默作为废物。
...
###### 干燥水分损失（`brazil-nut_drying_water_loss`）

根据接收和调制状态的实测质量与水分计算巴西坚果批次移除的水；已有水分数据时不得使用未测量的通用干燥损失。
...
- 数量范围：干燥水分损失筛选范围
  ...
  - 基准类型：过程输出（`process_output`）
  - 证据类型：方法公式（`method_formula`）
  - 来源：`mass-balance-identity`; `fao-brazil-nut-minor-oil-crops`

###### 可选调制废水（`brazil-nut_conditioning_residuals`）
...
### 过程：巴西坚果储存和交接（`brazil-nut_storage_and_gate_handoff`）

本过程为条件过程。当产品在声明交接点前储存，或数据包代表干燥、清理、分级和储存后的带壳批次而不是鲜品农场交接点参考状态时纳入。

#### 输入

##### 产品流

###### 储存用合格分级带壳批次（`brazil-nut_storage_input`）
...
- 基准类型：参考流（`reference_flow`）
- 证据类型：采集记录（`collected_record`）
- 采集协议：`cp_brazil-nut_storage_records`
- 数量范围：储存输入质量校验范围
  - 范围角色：QA 校验范围（`qa_guardrail`）
  - 下限：900
  - 上限：1,050
...
###### 储存包装和搬运服务（`brazil-nut_storage_packaging`）
...
###### 储存电力和搬运能源（`brazil-nut_storage_energy`）
...
###### 储存损伤和质量损失（`brazil-nut_storage_loss`）

按储存时间、原因和去向记录受损、霉变、虫害、洒漏或其他损失的带壳巴西坚果，损失不得计入交接输出。
...
###### 声明交接点带壳巴西坚果输出（`brazil-nut_gate_output`）
...
| --- | --- | --- | --- |
| `allocation_01` | 有意巴西坚果木质荚果输出的巴西坚果园和采收 | 当巴西坚果园、采收、苹果收集和巴西坚果收集有独立记录时，优先采用过程细分。巴西坚果木质荚果被有意收集并交付为副产品时，按实测干物质质量分配共用巴西坚果园和采收负荷；没有干物质数据时使用实测接收质量，并披露局限。 | `iso-14044-2006`; `fao-brazil-nut-india-production-1998` |
| `allocation_02` | 巴西坚果木质荚果不作为产品收集 | 苹果留在巴西坚果园或作为没有有意产品交接的残余处理时，不分配副产品信用；保留其质量和去向，并将共用负荷分配给有意生产的带壳巴西坚果输出。 | `iso-14044-2006`; `fao-brazil-nut-minor-oil-crops` |
...
| `allocation_04` | 多年生巴西坚果园期间和阶段 | 将建立、未结果年份、补植和清除按巴西坚果园寿命及合格产出基准年度化；每项投入和输出连接到作物年度或生命周期阶段，不得在原巴西坚果园和补植阶段重复计入补植事件。 | `fao-brazil-nut-india-production-1998`; `iso-14044-2006` |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `cp_brazil-nut_forest_lifecycle_records` | `brazil-nut_forest_establishment_and_management` | 巴西坚果园资产、土地、种植、补植、残余 | 巴西坚果园地块和资产登记 | 地块面积；建立年份；品种；树数；补植；清除；修剪残余；巴西坚果园阶段；合格产出 | 地块登记、田间记录和年度核对 | ha、item、kg、巴西坚果园年 | 事件和年度 | 完整声明寿命或有文件支持的代表性期间 | 所有贡献地块 | 将地块记录连接到作物年度和合格产出 | 签署登记、GIS 或测量面积、补植证据 |
| `cp_brazil-nut_forest_input_records` | `brazil-nut_forest_establishment_and_management` | 养分、改良剂、植保 | 投入施用记录 | 产品名；配方；养分含量；有效成分；数量；日期；地块；用途；施用方法 | 发票、施用日志和供应商记录 | kg 产品、kg 养分、kg 有效成分 | 每次施用 | 作物年度 | 所有贡献地块 | 按产品、养分、地块和作物年度汇总 | 发票、产品标签、施用日志 |
| `cp_brazil-nut_irrigation_records` | `brazil-nut_forest_establishment_and_management` | 灌溉和水 | 灌溉计量或水量台账 | 水源；体积；地块；日期；计量表；水质；取水和回用 | 计量表、泵记录或透明水量平衡 | m3 或 kg | 事件和月度 | 作物年度 | 所有灌溉地块 | 核对交付、取水和声明消耗 | 计量校准、泵记录、水量台账 |
| `cp_brazil-nut_energy_records` | 所有田间、采收、调制和储存过程 | 能源和载体 | 燃料、电力和设备记录 | 载体；数量；设备；作业；批次或地块；日期；时长；换算因子 | 发票、计量、燃料日志或设备记录 | L、kg、MJ 或 kWh | 事件和月度 | 作物年度及声明后处理期 | 每项作业和设施 | 按载体、作业、批次和期间汇总 | 发票、计量、燃料日志、设备规格 |
| `cp_brazil-nut_emission_records` | `brazil-nut_forest_establishment_and_management` | 直接和间接田间排放 | 排放计算输入记录 | 养分投入；残余；土壤条件；植保记录；排放物种；方法；因子 | 连接原始记录的计算工作簿 | kg 物质 | 年度和事件 | 作物年度 | 所有贡献地块 | 保留物质、接收介质、因子和来源 | 计算复核、因子版本、来源引用 |
| `cp_brazil-nut_harvest_lot_records` | `brazil-nut_harvest_and_collection` | 采收、收集、田间损失、残余 | 采收批次登记 | 地块；收集轮次；成熟度；采收质量；未收集质量；碎屑；苹果分离；去向 | 地磅、田间秤和批次日志 | kg、日期、批次 | 每次收集轮次 | 采收季 | 所有贡献地块 | 核对可采收、收集、损失和残余质量 | 计量校准、批次单、田间检查 |
| `cp_brazil-nut_husk_destination_records` | `brazil-nut_harvest_and_collection` | 巴西坚果木质荚果副产品或残余 | 去向和转移记录 | 苹果质量；干物质或水分；去向；接收方；价格或用途；去向；转移日期 | 称量、转移记录和去向确认 | kg 和声明分数 | 每次转移 | 采收季 | 每个苹果去向 | 分开记录有意输出与残余和废物 | 收据、转移单、去向证据 |
| `cp_brazil-nut_dehusking_conditioning_records` | `brazil-nut_primary_conditioning_and_grading` | 干燥、清理、用水、能源、水分 | 调制批次记录 | 输入质量；输入水分；方法；时长；能源；用水；输出水分；输出质量；清理损失；废水 | 批次日志、计量、水分检测和水量平衡 | kg、%、小时、L、kWh | 每批次 | 声明后处理期 | 每个调制设施或农场单元 | 将所有投入和输出连接至一个批次和交接点 | 校准秤、水分检测、批次单 |
| `cp_brazil-nut_grade_reject_records` | `brazil-nut_primary_conditioning_and_grading` | 等级、降级、拒收、异物 | 检验和分级记录 | 批次；等级；壳状况；仁缺陷；空壳；霉变；虫害；异物；合格质量；返工；去向 | 检验、抽样和称量记录 | kg、%、等级代码 | 每批次 | 声明后处理期 | 每个分级点 | 等级和拒收状态互斥 | 检验单、抽样方案、秤校准 |
| `cp_brazil-nut_storage_records` | `brazil-nut_storage_and_gate_handoff` | 储存输入、损失、包装、交接输出 | 库存和发货台账 | 期初库存；收货；批次；水分；储存时间；包装；能源；损伤；期末库存；发货质量；交接点 | 库存台账、发货单和储存日志 | kg、%、天、kWh、item | 收货、检查和发货 | 声明储存期 | 每个储存设施 | 期初 + 收货 = 发货 + 期末 + 损失 | 库存核对、发货单、水分和虫害记录 |

### 计算规则

| rule_id | 适用对象 | 公式或规则 | 输入 | 输出 | source_ids |
| --- | --- | --- | --- | --- | --- |
| `calc_01` | 多年生巴西坚果园投入 | 按巴西坚果园寿命基准和各作物年度合格产出分配建立、未结果年份、补植和清除。 | 巴西坚果园资产登记；巴西坚果园寿命；作物年度产出 | 每参考流的年度化巴西坚果园投入 | `fao-brazil-nut-india-production-1998`; `iso-14044-2006` |
| `calc_02` | 所有采收、调制和储存批次 | `期初质量 + 投入 = 合格输出 + 副产品 + 拒收物 + 残余 + 实测损失`；调查未解释差额。 | 批次质量和去向 | 核对后的质量平衡 | `mass-balance-identity` |
| `calc_03` | 鲜品到干燥或调制批次 | 干物质 = 湿质量 ×（1 − 水分分数）；扣除非水损失后，移除水 = 输入湿质量 − 输出湿质量。 | 输入/输出质量和实测水分 | 水分修正质量和干燥水损失 | `mass-balance-identity`; `fao-brazil-nut-minor-oil-crops` |
| `calc_04` | 巴西坚果木质荚果副产品 | 苹果有意交付时，按实测干物质质量分配共用巴西坚果园和采收负荷；无意交付时建模为残余去向，不分配副产品信用。 | 巴西坚果质量；苹果质量；水分或干物质；声明去向 | 分配后的负荷和副产品份额 | `iso-14044-2006` |
...
| `validation_03` | 多年生生产 | 巴西坚果园建立、未结果期、结果期、补植和清除连接至声明作物年度，且不得重复计量。 | `fao-brazil-nut-india-production-1998`; `iso-14044-2006` |
| `validation_04` | 采收和巴西坚果木质荚果分离 | 采收、收集、未收集、巴西坚果木质荚果、残余和合格巴西坚果质量互相核对，并声明苹果去向。 | `mass-balance-identity` |
| `validation_05` | 调制和质量 | 鲜品输入、调制水分、高等级、标准等级、降级/返工、拒收、异物、水分损失和废水互斥并满足质量平衡。 | `mass-balance-identity`; `fao-brazil-nut-minor-oil-crops` |
...
| excluded_use | 去壳仁生产、荚果开启、巴西坚果壳液回收、烘烤、蒸煮、去皮、巴西坚果仁加工、零售产品、消费者使用、交接点后物流以及未声明产品类别 |
...
| --- | --- | --- | --- |
| `fao-brazil-nut-india-production-1998` | official_guidance | FAO, Integrated Production Practices of Brazil nut in India, https://www.fao.org/4/ac451e/ac451e04.htm | 多年生巴西坚果园阶段、采收成熟度、巴西坚果木质荚果分离、干燥和巴西坚果园寿命 |
| `fao-brazil-nut-postharvest-2020` | extension_guidance | FAO TECA / African Brazil nut Initiative, Guide to harvesting and post-harvest handling of brazil-nut nuts, https://www.fao.org/family-farming/detail/en/c/1619106/ | 采收质量、采后处理、储存和过程分解 |
| `fao-brazil-nut-minor-oil-crops` | handbook | FAO, Minor oil crops: brazil-nut nut monograph, https://www.fao.org/4/x5043e/x5043e06.htm | 巴西坚果木质荚果分离、日晒、含水率背景和储存实践 |
| `fda-brazil-nut-nut-methods` | official_guidance | U.S. FDA, MPM V-10 Nuts and Nut Products Methods, https://www.fda.gov/food/laboratory-methods-food/mpm-v-10-nuts-and-nut-products-methods | 缺陷、霉变、虫害、壳和带壳质量区分 |
