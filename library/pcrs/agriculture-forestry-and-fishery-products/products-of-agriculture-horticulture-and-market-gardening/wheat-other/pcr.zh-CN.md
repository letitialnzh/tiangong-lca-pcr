...
---
# 声明农场门或初级处理门的非播种用小麦谷物
## 1. 适用范围与适用性
本 PCR 用于构建 CPC 3.0 `01112`“Wheat, other”对应的非播种用小麦谷物前景数据包。产品可用于食品、饲料或工业用途，但具体终端用途必须声明。范围覆盖年度小麦种植、收获，以及在声明门点位于收获后时所包含的初级干燥、清理、分级和短期储存，终点为声明的农场门或初级处理门。
本 PCR 不覆盖播种用小麦种子、育种或认证种子、面粉、麦芽、淀粉、乙醇及其他小麦加工产品、零售产品，也不覆盖下游食品、饲料或工业转化。“other”只是 CPC 覆盖标签，不是独立产品事实；数据包必须声明终端用途类别和实物状态。
...
| covered_products | 用于已声明食品、饲料或工业用途的非播种用小麦谷物，位于收获农场门或声明的初级处理门 |
| excluded_products | 播种用小麦种子；育种、基础、登记或认证种子；面粉；麦芽；淀粉；乙醇；其他小麦加工产品；零售即售产品 |
| representative_product | 在声明水分基准、农场门或初级处理门的整粒非播种用小麦谷物 |
| production_route | 年度小麦种植、收获，以及可选的收获后初级干燥、清理、分级和短期储存 |
| market_state | 已声明用途、质量描述、水分基准和门点的散装或包装非播种用小麦谷物 |
## 3. 参考流
| 字段 | 值 |
| --- | --- |
| What | 声明农场门或初级处理门的非播种用小麦谷物 |
| How much | 1 kg |
| How well | 已声明终端用途（食品、饲料或工业）、重要时的小麦类别或品种、水分基准、等级或质量描述、清理状态、地理范围、作物年度和门点 |
| How long or cycle | 一个声明的作物周期和收获年度；门点包含储存时另行声明储存时长 |
| reference_flow_link | 见下方参考数量和产品流 |
| 字段 | 值 |
| --- | --- |
| Reference amount | 1 kg |
| Reference product flow | 调质小麦籽粒 `78bd12e1-801c-4f96-a880-dba39e22783c` |
| Reference flow property | Mass `93a60a56-a3c8-11da-a746-0800200b9a66` |
| Reference unit group | Units of mass `93a60a57-a4c8-11da-a746-0800200c9a66` |
| Reference unit | kg |
| Binding | fixed |
| Required qualifiers | 非播种用途；食品/饲料/工业声明；重要时的小麦类别或品种；水分基准；质量或等级描述；清理/干燥状态；地理范围；作物年度；声明门点；散装或包装状态 |
构建前景数据包时，`Required qualifiers` 中的项目必须写入数据集元数据、过程说明、参考流注释、产品描述或等效数据包字段。通用 Tiangong 小麦身份由本 PCR 的非播种用产品边界限定；不得据此把数据集理解为播种用小麦种子。
## 4. 计量与单位规则
| 规则编号 | 适用对象 | 必需流属性 | 必需单位 | 规则 |
| --- | --- | --- | --- | --- |
| `reference_mass` | 参考产品 | Mass `93a60a56-a3c8-11da-a746-0800200b9a66` | kg | 参考流必须表示为所选声明门点的非播种用小麦谷物净质量，并声明水分基准。 |
...
| `boundary_harvest` | 收获和田间交接 | 当这些活动属于前景数据包时，包含收获、脱粒或联合收割、田间收集，以及到声明农场门或初级处理交接点的运输。 | `fao-grain-postharvest-systems-1998`; `fao-wheat-postharvest-operations-1999` |
| `boundary_primary_conditioning` | 收获后路线 | 只有当声明门点位于这些作业之后时，才包含初级干燥、清理、分级、粉尘或筛下物处理及短期储存。 | `fao-wheat-postharvest-operations-1999`; `fao-grain-postharvest-systems-1998` |
| `boundary_downstream_exclusion` | 下游加工 | 排除制粉、制麦、淀粉或乙醇转化、食品或饲料配方、零售和消费者使用；这些应由下游数据集表示。 | `fao-wheat-postharvest-operations-1999`; `usda-wheat-standards-2020` |
| `boundary_input_completeness` | 所有前景路线 | 对肥料、灌溉、植保、田间能源、电力、土地、收获能源、干燥能源、清理损失、残余物和直接排放给出数值、合理的零值或有记录的排除理由。 | `ipcc-2019-managed-soils-n2o`; `fao-wheat-postharvest-operations-1999` |
### 边界概化
| 字段 | 值 |
| --- | --- |
...
| product_classification_scope | 对应 CPC 3.0 `01112`“Wheat, other”的非播种用小麦谷物；播种用小麦种子是上游投入，不是参考产品 |
| recursive_input_rule | 作为作物建立投入使用的同类小麦流，应记录为播种用种子投入并披露来源、批次、数量和上游种子数据集；不得将其静默追踪为非播种用参考产品 |
| upstream_dataset_requirement | 播种用种子、肥料、植保产品、能源、水供应、运输、干燥、废物处理及其他上游投入使用已识别数据集或声明的供应商记录 |
| disclosure | 声明终端用途、重要时的小麦类别或品种、作物周期、地理范围、作物年度、播种用种子来源、灌溉状态、水分基准、收获和处理路线、门点、质量描述以及所有非参考输出的去向 |
## 6. 过程清单结构
### 过程图
...
### 过程：小麦作物生产（`wheat_crop_production`）
#### 输入
##### 产品流
###### 用于小麦作物的播种用种子（`seed_for_sowing_input`）
播种用种子是作物建立的上游投入，不是参考产品。记录其来源和批次身份，不得把由此得到的非播种用谷物数据集标为播种用小麦种子。
...
- 归一化基准：每 1,000 kg 声明门点收获小麦谷物
- 基准类型：过程输出（`process_output`）
- 证据类型：采集记录（`collected_record`）
- 采集协议：`cp_seed_input_records`
###### 农业养分与肥料投入（`wheat_crop_production_agricultural_nutrient_inputs`）
...
#### 输出
##### 产品流
###### 收获的非播种用小麦谷物（`harvested_wheat_grain`）
收获小麦谷物是交给农场门或初级处理路线的输出。它仍是非播种用谷物过程输出；只有当声明产品状态与已核实的初级处理平台流一致时，才使用固定参考 UUID。
- 选定流：田间交接点的收获非播种用小麦谷物
- 流属性/单位：Mass / kg
...
###### 小麦秸秆或田间残余物（`wheat_straw_or_field_residue`）
...
#### 输入
##### 产品流
###### 收获时接收的站立或已收小麦作物（`wheat_crop_received_for_harvest`）
收获节点接收作物生产路线的产品，并分别记录收获谷物和残余物结果。
 - 选定流：收获或站立小麦作物物料 12da5e7d-9b93-4404-8c7d-08f98bec6238
- 流属性/单位：Mass / kg
...
#### 输入
##### 产品流
###### 进入初级处理的收获小麦谷物（`grain_to_primary_conditioning`）
只有当声明门点位于初级干燥、清理、分级或短期储存之后时，才使用本输入。
 - 选定流：进入初级处理的小麦谷物 12da5e7d-9b93-4404-8c7d-08f98bec6238
- 流属性/单位：Mass / kg
...
- 数值来源模式：前景记录（`foreground_record`）
- 适用范围：技术特定（`technology_specific`）
- 归一化基准：每 kg 处理后合格小麦谷物输出
- 基准类型：过程输出（`process_output`）
- 证据类型：采集记录（`collected_record`）
- 采集协议：`cp_conditioning_energy_records`
- 来源：`fao-wheat-postharvest-operations-1999`
##### 废物流
...
- 来源：`fao-wheat-postharvest-operations-1999`; `usda-wheat-standards-2020`
#### 输出
##### 产品流
###### 处理后的非播种用小麦谷物（`conditioned_wheat_grain`）
当声明门点位于初级处理之后时，处理后的合格小麦谷物是参考输出。
- 选定流：调质小麦籽粒 `78bd12e1-801c-4f96-a880-dba39e22783c`
- Binding: fixed
- 流属性/单位：Mass / kg
...
#### 输出
##### 产品流
###### 声明门点的非播种用小麦谷物（`declared_gate_wheat_grain`）
...
| `allocation_grain_and_residue` | 小麦谷物和移出的秸秆或残余物 | 根据声明去向处理留在田间的残余物。若秸秆或其他残余物被移出或销售且不能拆分过程，使用明确声明的物理或经济分配方法，并披露敏感性。 | `iso-14044-2006`; `fao-wheat-postharvest-operations-1999` |
| `allocation_screenings_and_rejects` | 筛下物、破损谷物和不合格物料 | 将合格谷物、筛下物、拒收物和损失作为不同输出记录并声明去向。没有明确返工或分配决定时，不得将拒收物料计入参考产品。 | `fao-wheat-postharvest-operations-1999`; `usda-wheat-standards-2020` |
...
| --- | --- | --- | --- | --- | --- |
| `normalize_to_declared_gate` | 所有前景流 | 记录的流数量 / 所选声明门点的合格非播种用小麦谷物质量 × 参考数量 | 所有适用协议；声明门点输出 | 每 1 kg 参考产品的数量 | `mass-balance-identity` |
| `convert_moisture_basis` | 谷物质量比较 | 干物质质量 = 原始质量 × (1 - 水分比例)；保留原始值、转换值和实测水分基准 | 收获和处理质量；水分记录 | 可比质量基准 | `fao-wheat-postharvest-operations-1999`; `usda-wheat-standards-2020` |
...
| `reconcile_conditioning_outputs` | 初级处理 | 处理后合格谷物 + 筛下物 + 拒收物 + 粉尘或实测损失 = 同一水分基准下的处理投入；解释剩余项 | `cp_conditioning_reject_records`; `cp_conditioned_output_records` | 处理收率和损失核对 | `fao-wheat-postharvest-operations-1999`; `mass-balance-identity` |
...
| --- | --- | --- | --- |
| `dq_non_seed_identity` | 参考产品 | 声明参考产品为非播种用小麦谷物，并写明食品、饲料或工业用途；不得用种子质量声明替代。 | 产品元数据、合同、质量记录或发运记录 |
...
| 规则编号 | 适用对象 | 规则 | 来源 |
| --- | --- | --- | --- |
| `validation_non_seed_boundary` | 产品身份 | 确认参考产品为非播种用小麦谷物，并确认播种用种子、面粉、麦芽、淀粉、乙醇和下游产品已排除或单独建模。 | `usda-wheat-standards-2020`; `fao-wheat-postharvest-operations-1999` |
| `validation_reference_flow` | 参考流 | 确认一个 1 kg 参考流使用所选 Wheat 身份、质量属性、kg 单位，并声明终端用途、水分基准、质量描述、地理范围、作物年度和门点。 | `usda-wheat-standards-2020` |
| `validation_process_completeness` | 过程图和库存 | 确认作物生产、收获以及适用的初级处理或储存过程已经纳入，或已根据声明门点明确排除。 | `fao-grain-postharvest-systems-1998`; `fao-wheat-postharvest-operations-1999` |
...
| `validation_moisture_and_mass_balance` | 收获和处理 | 确认质量核对使用共同水分基准，且合格谷物、残余物、筛下物、拒收物和损失均有声明去向。 | `fao-wheat-postharvest-operations-1999`; `mass-balance-identity` |
...
| excluded_use | 播种用小麦种子；面粉；麦芽；淀粉；乙醇；加工食品或饲料；门点不兼容或物料损失/去向未解决的数据集 |
| required_metadata | 参考流；非播种用途；重要时的小麦类别或品种；地理范围；作物周期和年度；播种用种子来源；灌溉状态；肥料和植保记录；水分基准；质量描述；收获和处理路线；声明门点；残余物和拒收物去向；分配；质量披露 |
...
| --- | --- | --- | --- |
| `fao-wheat-postharvest-operations-1999` | official_guidance | FAO INPhO，*WHEAT: Post-harvest Operations*，<https://www.fao.org/fileadmin/user_upload/inpho/docs/Post_Harvest_Compendium_-_WHEAT.pdf> | 小麦收获、干燥、清理、储存、损失和初级处理过程分解 |
...
| `usda-wheat-standards-2020` | official_guidance | USDA Agricultural Marketing Service，*United States Standards for Wheat*，<https://www.ams.usda.gov/sites/default/files/media/WheatStandards.pdf> | 小麦谷物质量、水分和等级描述，以及与种子语义区分 |
